    ...
    topics: [
      {
        title: 'Sessions',
        body: [
          'Search and filter every logged session at once — free text across notes, scenario, compulsions, and techniques, plus hierarchy, response to urges, exposure type, and date-range filters. Sort by date or peak SUDS.',
      ...
      },
      {
        title: 'Live',
        body: [
          'Run an ERP exposure in real time. If you have any Fear Ladders set up, setup defaults to picking one and a rung from it; a "Custom exposure" toggle switches to typing the hierarchy and rung manually instead — either way you rate how distressed or uncomfortable you feel right now, then start the timer.',
          ...
          'While running, the main screen keeps the focus on the timer, the scenario, a values reminder if you\'ve written one, and the optional support card — not on numbers. SUDS logging (the rating buttons, a precise custom value, the live curve, and readings logged so far) lives behind a "SUDS tracking (optional)" section, collapsed by default; tap it open any time you want to log a reading, and collapse it again just as easily.',
          'If the hierarchy you type in Custom mode matches a planned Fear Ladder, its rungs still show up as tappable rows — picking one fills in the rung number, scenario, and expected difficulty for you, though you can still edit any of it afterward.',
          'Hierarchy and Rung also suggest matches from your own past sessions as you type or focus the field (Rung narrows to whatever rungs you\'ve logged under the hierarchy you picked), and previously-used expected-difficulty ranges for that hierarchy show up as tappable chips — all drawn from sessions already on this device, nothing built in.',
          '"🦊 Rough stretch? Something to lean on" opens a card offering a random defusion technique or a fox fact — something to hold onto while you keep going, not a suggestion to stop, since leaving early because it\'s uncomfortable would just be another compulsion. It also opens on its own if SUDS peaks at 8+ and stays there for 15+ minutes; closing it only suppresses that one occurrence, not the next rough stretch.',
          'Stop the exposure to fill in the rest (compulsions, techniques, notes) and save — the full SUDS curve is shown on this wrap-up screen, and on the session\'s page afterward, and updates live if you correct a reading before saving.',
    ...
    topics: [
      {
        title: 'Journal',
        body: [
          'Five ways to journal. "Morning Anchor" (5–10 min) and "OCD Wind Down" (10 min) are structured, timed prompts grounded in sleep and anxiety research, with built-in compulsion-warning signs.',
          '"Quick prompt" picks one short prompt at random from a pool spanning general intrusive thoughts, harm OCD, sexual intrusive thoughts, ROCD, postpartum, and Defusion & Values — reroll for a different one, then free-write underneath. None of them ask you to prove a thought true, false, or safe — they\'re built to help you notice, tolerate uncertainty, and choose what matters next instead.',
          '"Intrusive thought" is a one-tap capture for noticing a thought without engaging it: tag which theme is the closest fit — it doesn\'t have to be exact — and save. There\'s no text field on purpose, since writing the thought out is still a way of engaging with it. Saving one shows a "Caught the fox!" moment instead of the usual confirmation, since noticing without acting is the whole point — plus a lifetime count of how many times that\'s happened, not a daily streak.',
          '"Obsessive Thought" is an OCD-specific worksheet for one obsession, moving through five fixed steps one at a time — name the obsession, what OCD is demanding, spot the trap (the process it\'s running, not whether the content is true), practice a short uncertainty statement, and choose a next action — each with its own countdown that prompts you (without forcing you) to move on once it hits zero, all inside an overall 30-minute hard cap. A 0–10 "urge to solve/check" rating is taken before and after, and is explicitly not expected to go down. There\'s no way back to an earlier step once you\'ve moved on, on purpose — the point is writing it once, not revising it. Entries saved before this worksheet changed still show their original evidence-for/against structure and believability rating when you reopen them.',
    ...
    topics: [
      {
        ...
        body: [
          ...
          'Step 5 (the debrief — what showed up, what you practiced instead of compulsions, what helped you continue, what you\'ll try next time) is meant to be filled in after you\'ve actually attempted the task, which might be hours or days later. SUDS ratings are optional and collapsed by default. Plans save with "Debrief pending" and reopen for editing at any time — you don\'t have to finish it in one sitting. "Flag for your OCD therapist if" names process signs (planning becoming a way to delay, the worksheet turning into a ritual, avoidance increasing) rather than a SUDS threshold.',
      ...
      },
      {
        title: 'Fear Ladders',
        body: [
          'Plan a hierarchy\'s rungs — a number, a short description, and an optional expected difficulty (0–10) each — before you\'ve ever logged a session in it. Normally a hierarchy only exists once a session references it; this is the way around that. Expected difficulty is a planning estimate, not a distress level you need to reach.',
          ...
          'It also shows up on the Live tab: pick a ladder in the planned setup flow (or type a hierarchy that matches one in Custom mode) and its rungs appear as a pickable list, autofilling the scenario and expected difficulty.',
